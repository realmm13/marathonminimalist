...
describe('Tempo Run Algorithm', () => {
  describe('Basic Functionality', () => {
    test('should generate tempo run for week 1', () => {
      ...
      expect(result.targetPace).toBe('8:01/mi'); // Marathon pace for 3:30:00
      expect(result.description).toContain('marathon pace');
    });
    ...
    test('should generate tempo run for peak week (week 10)', () => {
      ...
      expect(result.targetPace).toBe('8:01/mi');
    });
  ...
  describe('Pace Calculations', () => {
    ...
    test('should handle edge case marathon times', () => {
      ...
      expect(fastResult.targetPace).toBe('5:43/mi');
      ...
      expect(slowResult.targetPace).toBe('11:27/mi');
    });
  });
  ...
  describe('Unit Conversions', () => {
    ...
    test('should convert pace to min/km when requested', () => {
      ...
      // 8:01/mile = ~4:59/km
      expect(result.targetPace).toBe('4:59/km');
    });
  });
  ...
  describe('Consistency Checks', () => {
    test('should maintain consistent pace across all weeks', () => {
      ...
      // All paces should be the same (marathon pace)
      const uniquePaces = [...new Set(paces)];
      expect(uniquePaces).toHaveLength(1);
      expect(uniquePaces[0]).toBe('8:01/mi');
    });
